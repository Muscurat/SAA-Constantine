...
$dure = $_POST['dure'];
        ...
        $mois_nai = $_SESSION['mois_nai'];
        $annee_nai = $_SESSION['annee_nai'];
        $sexe = $_SESSION['sexe'];
        $adr = $_SESSION['adr'];
        $profession = $_SESSION['profession'];
        $num_permit = $_SESSION['num_permit'];
        $date_permit = $_SESSION['date_permit'];
        $mois_permit = $_SESSION['mois_permit'];
        $annee_permit = $_SESSION['annee_permit'];
        $datePermit = $annee_permit.'-'.$mois_permit.'-'.$date_permit;
        $dateNai = $annee_nai.'-'.$mois_nai.'-'.$date_nai;
       ...
       try {
            ...
            die ('Erreur : ' . $e->getMessage());
                ...
                if($exist || $exist2){
                   ...
                   $conn->query('INSERT INTO user(pseudo,password,email,nom,prenom,num_tel,sexe,role,
                   etat,avatar) 
                       values("'.$pseudo.'","'.$password.'","'.$email.'","'.$nom.'","'.$prenom.'","'.$tel.'","'.$sexe.'"
                             ,"'.$role.'","'.$etat.'","default-avatar.gif")');

                    $_SESSION['nomUser'] = $nom;
                    $_SESSION['prenomUser'] = $prenom;
                    $_SESSION['clientbloque'] = true;    
                    
                    $result = $conn->query('select id_user from user where pseudo="'.$pseudo.'" and password="'.$password.'"');
                    $result = $result->fetch();
                    $id_user=$result["id_user"];
                    $_SESSION['id_user']=$id_user;
                    $conn->query('INSERT INTO client(id_user,date_naiss,adr,prof,num_permit,date_permit)
                    values("'.$id_user.'","'.$dateNai.'","'.$adr.'","'.$profession.'","'.$num_permit.'","'.$datePermit.'")');  

                  $result = $conn->query('SELECT id_client FROM client where id_user="'.$id_user.'"');
                  $result= $result->fetch();
