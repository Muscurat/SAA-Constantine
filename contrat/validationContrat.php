...
$date_dem = $_POST['date_dem'];
       ...
       try {
        ...
        $etat = 2;
        $result2 = $conn->query('SELECT user.id_user,user.etat FROM user
        INNER JOIN client ON client.id_user=user.id_user 
        WHERE client.id_client="'.$id_client.'"');
        $result2 = $result2->fetch();
        $id_user = $result2['id_user'];
        $etatOrg = $result2['etat'];

        if($etatOrg==0){
                
            $conn->query('UPDATE user set etat="'.$etat.'" WHERE id_user="'.$id_user.'"');            
        
        }

        $conn->query('UPDATE vehicule SET num_chass="'.$num_chass.'",imat="'.$immat.'",
          nom="'.$nom_veh.'",marque="'.$marque.'",genre="'.$genre.'",
            usag="'.$usage.'",zone="'.$zone.'",energie="'.$energie.'",
              puiss="'.$puissance.'",nbr_place="'.$nbr_place.'",valeur="'.$valeur_veh.'" WHERE
